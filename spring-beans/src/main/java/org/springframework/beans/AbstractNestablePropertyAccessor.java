...
public abstract class AbstractNestablePropertyAccessor extends AbstractPropertyAccessor {
	...
	public void setPropertyValue(PropertyValue pv) throws BeansException {
		...
		if (tokens == null) {
			...
			AbstractNestablePropertyAccessor nestedPa;
			try {
				nestedPa = getPropertyAccessorForPropertyPath(propertyName);	// 获取 BeanWrapper 对象属性
			}
			catch (NotReadablePropertyException ex) {
				throw new NotWritablePropertyException(getRootClass(), this.nestedPath + propertyName,
						"Nested property in path '" + propertyName + "' does not exist", ex);
			...
			nestedPa.setPropertyValue(tokens, pv);	// 开始属性填充
		}
		else {
	...
	private void processLocalProperty(PropertyTokenHolder tokens, PropertyValue pv) {
		PropertyHandler ph = getLocalPropertyHandler(tokens.actualName);	// 获取 setter、getter 方法
	...
	private Object convertIfNecessary(@Nullable String propertyName, @Nullable Object oldValue,
		...
		try {
			return this.typeConverterDelegate.convertIfNecessary(propertyName, oldValue, newValue, requiredType, td);// TODO
		}
	...
	/**
	 * Recursively navigate to return a property accessor for the nested property path.( 属性的访问路径 )
